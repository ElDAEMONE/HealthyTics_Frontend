...
class IOTHistory extends StatefulWidget {
  const IOTHistory({super.key});

  @override
  State<IOTHistory> createState() => _IOTHistoryState();
}

class _IOTHistoryState extends State<IOTHistory> {
                                          ...
                                          content: Column(
                                            ...
                                            children: [
                                              ...
                                              getCard(
                                                ...
                                                rec['value']['humy'],
                                                symbol:
                                                    '%', // Added unit for Humidity percentage
                                              ),
                                              getCard(
                                                Icons.brightness_1,
                                                'SPO2',
                                                rec['value']['spo2'],
                                                symbol:
                                                    '%', // Added unit for SPO2 percentage
                                              ),
                                            ],
                                          ),
    ...
    if (list.isNotEmpty) {
      ...
      for (var element in list) {
        chartData1.add(ChartData(
            ...
            double.parse(element['value']['body_temp'].toString())));

        chartData2.add(ChartData(
            getDateTime(int.parse(element['value']['timestamp'].toString())),
            double.parse(element['value']['bpm'].toString())));

        chartData3.add(ChartData(
            getDateTime(int.parse(element['value']['timestamp'].toString())),
            double.parse(element['value']['spo2'].toString())));

        chartData4.add(ChartData(
            getDateTime(int.parse(element['value']['timestamp'].toString())),
            double.parse(element['value']['humy'].toString())));
      }
  ...
  Future<void> getData() async {
    _databaseReference
        .child(FirebaseStructure.iotHistory)
        .once()
